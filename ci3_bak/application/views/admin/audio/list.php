...
<table class="table table-bordered table-hover">
    ...
    <tr>
        <th colspan="4" class="text-center">
            <a href='<?=ci3_url('admin/audio/edit')?>' type="button" id='addBtn' class="btn btn-success">创建</a>
            &nbsp;&nbsp;&nbsp;&nbsp;
    ...
    <?php 
        if(!empty($dataModel)){
            foreach ($dataModel as $key => $value) {
                $tdBody = html_a(['href'=>ci3_url('admin/audio/edit',['id'=>$value['id']]),'text'=>'修改','class'=>'edit btn btn-primary btn-xs']);
                $tdBody .= str_repeat('&nbsp;',4);
...
$(document).ready(function() {
    $('#searchBtn').click(function(){
        var title = $('#title').val();
        var url = "<?=ci3_url('admin/audio/index')?>?title="+title;
        $.common.location(url);
        return false;
    })
    ...
    $('.deleteBtn').click(function(){
        ...
        $.post("<?=ci3_url('admin/audio/delete')?>",{'id':id},function(dt){
            $.common.alert(dt);
            $.common.location("<?=ci3_url('admin/audio/index',['type'=>$audioType])?>");
        })
    })
})
</script> 
